...
@router_member.route("/index", methods=['GET', 'POST'])
def index():
    resp_data = {}
    req = request.values
...
@router_member.route("/comment", methods=['GET', 'POST'])
def comment():
    resp_data = {}
    req = request.args
    page = int(req['p']) if ('p' in req and req['p']) else 1
...
@router_member.route("/info", methods=['GET', 'POST'])
def info():
    resp_data = {}
